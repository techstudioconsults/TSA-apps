import { MarketingCycle, MarketingCycleFormData } from '@/schemas/marketing-cycle.schema';

const API_URL = `${process.env.NEXT_PUBLIC_API_URL}/marketing-cycles`;

export async function fetchMarketingCycles(token: string): Promise<MarketingCycle[]> {
...
export async function createMarketingCycle(data: MarketingCycleFormData, token: string) {
  const response = await fetch(API_URL, {
    method: 'POST',
    headers: {
...
export async function updateMarketingCycle(id: string, data: Partial<MarketingCycleFormData>, token: string) {
